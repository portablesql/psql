...
type StructField struct {
	...
	autoinc     bool // identity / auto-increment column (autoinc attribute)
}

// IsAutoInc reports whether the column is an identity (auto-increment)
// column, declared with the autoinc attribute:
//
//	ID uint64 `sql:",key=PRIMARY,autoinc"`
//
// The attribute is kept as autoinc=1 in the attributes returned by
// [StructField.GetAttrs], so that a dialect's [TypeMapper].FieldDef can render
// the column definition: AUTO_INCREMENT on MySQL/MariaDB, GENERATED BY DEFAULT
// AS IDENTITY on PostgreSQL/CockroachDB, and nothing on SQLite where an
// INTEGER PRIMARY KEY column is the rowid and is generated automatically.
//
// [Insert], [InsertIgnore], [Replace] and [BulkInsert] omit the column from
// the statement while the field is zero (or a nil pointer), so the database
// generates the value, and populate the field afterwards from RETURNING or
// LastInsertId. [Update] never writes an autoinc column.
func (f *StructField) IsAutoInc() bool {
	return f.autoinc
}
