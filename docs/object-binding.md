...
| `autoinc` | Identity column generated by the database (`AUTO_INCREMENT`, `GENERATED BY DEFAULT AS IDENTITY`, rowid), see [Identity columns](#identity-columns-autoinc). |
...
Inference only happens when the tag has no attribute other than `key`,
`autoinc` and `softdelete`: ``ID uint64 `sql:",key=PRIMARY,autoinc"` `` is
still inferred, while a field with `size=`, `null=` or `default=` must also
carry `type=` or `import=`.
...
(default), `FULLTEXT`, `SPATIAL`, `VECTOR`, `GIN`, `GIST`) and the quoted
`fields` list:
...
`GIN` and `GIST` are PostgreSQL index methods (jsonb containment, full-text
search, ranges); the other engines skip them. An `expression` attribute
replaces the `fields` list with an indexed expression in which `{Column}`
refers to a column. Because the tag parser strips quote characters, the
expression must be wrapped in double quotes (`\"` inside the Go tag);
single quotes inside it are preserved, double quotes cannot appear:
...
See [Advanced features](advanced.md#full-text-search) for full-text search
...
| Engine | PRIMARY | UNIQUE | INDEX | FULLTEXT / SPATIAL | VECTOR | GIN / GIST |
|--------|---------|--------|-------|--------------------|--------|------------|
| MySQL | inline | inline `UNIQUE INDEX name` | inline `INDEX name` | inline | ignored | ignored |
| PostgreSQL | inline | constraint `<table>_<name>` | `CREATE INDEX "<table>_<name>"` | ignored | `CREATE INDEX ... USING hnsw` (`method=`, `opclass=` attributes) | `CREATE INDEX ... USING gin/gist` on the columns or the `expression` (CockroachDB: inverted index for GIN) |
| SQLite | inline | `CREATE UNIQUE INDEX "<table>_<name>"` | `CREATE INDEX "<table>_<name>"` | ignored | ignored | ignored |
...
`autoinc` marks an integer column (or pointer to one; one per table)
generated by the database: `AUTO_INCREMENT` on MySQL/MariaDB,
`GENERATED BY DEFAULT AS IDENTITY` on PostgreSQL/CockroachDB, and nothing on
SQLite where an `INTEGER PRIMARY KEY` is the rowid. `Insert`, `InsertIgnore`,
`Replace` and `BulkInsert` omit the column while the field is zero and
populate it afterwards from `RETURNING` or `LastInsertId`; objects carrying
a value keep it. `Update` never writes it. Details in
[Advanced features](advanced.md#identity-columns-autoinc).
...
### Products and feature flags

`be.Engine()` groups wire-compatible products; `be.Variant()` names the
actual one (`psql.VariantPostgreSQL`, `VariantCockroachDB`, `VariantMySQL`,
`VariantMariaDB`, `VariantSQLite`) as detected by the driver, and
`be.ServerVersion()` its version string. `be.Supports(feature)` reports
whether a `psql.Feature*` constant (`FeatureReturning`,
`FeatureAdvisoryLocks`, `FeatureDistinctOn`, `FeatureAsOfSystemTime`,
`FeatureBulkCopy`, ...) is available on that product; operations using an
unavailable feature fail with an error wrapping `psql.ErrNotSupported`.

```go
if be.Supports(psql.FeatureReturning) {
    user, err = psql.RunQueryTOne[User](ctx, psql.B().Insert().Table("users").Set(values).Returning("*"))
} else {
    err = psql.Insert(ctx, user)
}
```

The feature matrix and the per-product rendering are in
...
// BulkInsert: many objects with as few statements as possible
err = psql.BulkInsert(ctx, users, psql.BulkBatchSize(1000))
```
...
- On engines whose driver reports `RETURNING` (PostgreSQL, CockroachDB) the
  statement uses it, and each object is refreshed with the stored row
  (server defaults included).
- Elsewhere, when the table has an `autoinc` column, or its primary key is
  a single integer column, and that field is still zero (or a nil pointer),
  it is populated from `LastInsertId` (`AUTO_INCREMENT` / rowid). Only an
  [`autoinc`](#identity-columns-autoinc) column is created with
  `AUTO_INCREMENT` on MySQL; a plain integer key relies on a column you
  created that way.
- An `autoinc` column whose field is zero is left out of the statement so
  the database generates it; objects carrying a value keep it, and one call
  may mix both kinds.
...
`psql.BulkInsert(ctx, rows, opts...)` writes a slice of objects with
PostgreSQL `COPY` where the driver provides it, and multi-row `INSERT`
statements elsewhere, in batches of `psql.BulkBatchSize(n)` rows (default
500, capped by the engine's parameter limit). `psql.BulkIgnore()` skips
conflicting rows and `psql.BulkNoHooks()` skips the hooks. Generated keys
are read back on PostgreSQL/CockroachDB (`RETURNING`) and MySQL/MariaDB
(`LastInsertId` + index) but not on SQLite. Hooks run per row, after each
batch for the `After*` ones; wrap the call in `psql.Tx` for all-or-nothing
behaviour. See [Advanced features](advanced.md#bulk-insert).
...
- `psql.Update(ctx, obj)` writes only the changed columns of tracked objects
  ...
  which case the `After*` hooks do not fire. It needs a main key. An
  `autoinc` column is never part of the `SET` clause.
...
| `psql.FetchLockShare`, `psql.FetchLockNoKeyUpdate`, `psql.FetchLockKeyShare`, `psql.WithLock(mode, tables...)` | Other row lock modes (`FOR SHARE`, `FOR NO KEY UPDATE`, `FOR KEY SHARE`, `FOR ... OF`), see [Advanced features](advanced.md#row-lock-modes) |
| `psql.AsOfSystemTime("-10s")` | CockroachDB historical read (`AS OF SYSTEM TIME`); also honored by `Count`. `psql.ErrNotSupported` elsewhere. |
