...
- Engine-specific SQL belongs in the driver modules through the optional dialect interfaces (`TypeMapper`, `KeyRenderer`, `UpsertRenderer`, `SchemaChecker`, `ErrorClassifier`, `VectorRenderer`, and in `dialect_ext.go`: `RetryableChecker`, `LockRenderer`, `LockTimeoutChecker`, `BulkInserter`, `ReturningStatements`, `VariantAware`); the core only switches on `Engine` for rendering details.
- Products sharing an engine are told apart by `Variant` (`variant.go`: `VariantPostgreSQL`/`VariantCockroachDB`, `VariantMySQL`/`VariantMariaDB`, `VariantSQLite`), set by the driver with `psql.WithVariant` / `psql.WithServerVersion`. Feature availability goes through `Backend.Supports(Feature*)` (`dialect_ext.go`: `defaultSupports` engine-level table, overridable by a `VariantAware` dialect); rendering a missing feature returns an error wrapping `ErrNotSupported` rather than emitting SQL the product rejects. Unit tests target a product without a server with `psql.NewBackend(engine, nil, psql.WithVariant(v))`.
- Advanced features live in: `builder_pg.go` (RETURNING, `Excluded`/`OnConflictConstraint`/`DoUpdateWhere`, CTEs, DISTINCT ON, lock modes, AS OF SYSTEM TIME, EXPLAIN, `InsertRows`/`Values`), `json_expr.go`, `fulltext.go`, `lock.go` (named locks), `core_bulk.go` (`BulkInsert`), `context.go` (`TxWithOptions`, retries), the `autoinc` attribute in `table.go`/`insert.go`/`update.go`. They are documented in `docs/advanced.md` (feature matrix per product) and exercised on every engine by `../psql-test/core_features_test.go`, which skips with `be.Supports(...)`.
