...
//
// An autoinc column (see [StructField.IsAutoInc]) is never part of the SET
// clause: identity values are assigned by the database and are normally the
// key the row is addressed by. Changing one requires a raw UPDATE.
func (t *TableMeta[T]) Update(ctx context.Context, target ...*T) error {
	...
	for _, obj := range target {
		...
		for _, f := range bt.fields {
			if f.IsAutoInc() {
				// identity columns are generated by the database, never SET
				continue
			}
